...
class Drone extends EventEmitter {
    ...
    takePicture() {
        this.network.writeTakePicture();
				this.emit("executionComplete");
				setTimeout(() => {
					this.emit('flightStatusChange', 'hovering');
				}, 100);
    }
    ...
    animate(animation) {
		...
		left(timeInMillis) {
				Logger.info(`Going left for ${timeInMillis} millis.`);
			  this.flightParams.roll = -this.inputSensitivity;
				setTimeout(() => {
		        this.flightParams.roll = 0;
						this.emit("executionComplete");
				}, timeInMillis);
		}

		right(timeInMillis) {
			  Logger.info(`Going right for ${timeInMillis} millis.`);
				this.flightParams.roll = this.inputSensitivity;
				setTimeout(() => {
						this.flightParams.roll = 0;
						this.emit("executionComplete");
		...
		forward(timeInMillis) {
			  Logger.info(`Going forward for ${timeInMillis} millis.`);
        this.flightParams.pitch = this.inputSensitivity;
				setTimeout(() => {
						this.flightParams.pitch = 0;
						this.emit("executionComplete");
				}, timeInMillis);
		}

		back(timeInMillis) {
				Logger.info(`Going back for ${timeInMillis} millis.`);
				this.flightParams.pitch = -this.inputSensitivity;
				setTimeout(() => {
						this.flightParams.pitch = 0;
						this.emit("executionComplete");
		...
		up(timeInMillis) {
			  Logger.info(`Going up for ${timeInMillis} millis.`);
				this.flightParams.altitude = this.inputSensitivity;
				setTimeout(() => {
						this.flightParams.altitude = 0;
						this.emit("executionComplete");
						setTimeout(() => {
							this.emit('flightStatusChange', 'hovering');
						}, 100);
		...
		down(timeInMillis) {
			  Logger.info(`Going down for ${timeInMillis} millis.`);
				this.flightParams.altitude = -this.inputSensitivity;
				setTimeout(() => {
						this.flightParams.altitude = 0;
						this.emit("executionComplete");
		...
		rotateLeft(timeInMillis) {
			  Logger.info(`Rotating left for ${timeInMillis} millis.`);
				this.flightParams.yaw = -this.inputSensitivity;
				setTimeout(() => {
						this.flightParams.yaw = 0;
						this.emit("executionComplete");
						setTimeout(() => {
							this.emit('flightStatusChange', 'hovering');
						}, 100);
		...
		rotateRight(timeInMillis) {
			  Logger.info(`Rotating right for ${timeInMillis} millis.`);
				this.flightParams.yaw = this.inputSensitivity;
				setTimeout(() => {
						this.flightParams.yaw = 0;
						this.emit("executionComplete");
